...
function lin_term(N)
    ...
    d = (1/2) * ones(N-2)
    d2 = (-1/2) * ones(N-2)
    DiffEqArrayOperator(-(1/dx^2) * diagm(-2 => d2, -1 => du, 1 => du2, 2 => d))
end
...
@time wp = WorkPrecisionSet(prob,abstols,reltols,setups;
                            ...
                            save_everystep=false, appxsol=test_sol, maxiters=Int(1e5));#324s
...
@time wp = WorkPrecisionSet(prob,abstols,reltols,setups;
                            print_names=true, names=labels,
                            numruns=5, error_estimate=:l2,
                            save_everystep=false, appxsol=test_sol, maxiters=Int(1e5));#31s
